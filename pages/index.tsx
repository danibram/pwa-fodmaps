  ...
  const handleChange = (event) => {
    ...
    if (event.target.value === "") {
      ...
      let filteredFoods = cards.foods.filter(
        (o) =>
          sanitizeStr(o.name).indexOf(event.target.value.toLowerCase()) > -1
      );
      setCardFiltered(filteredFoods);
      setCategories(
        Array.from(new Set(filteredFoods.map(({ category }) => category)))
      );
    }
  };
          ...
          {categoriesFiltered.map((category) => (
            <li key={`section-${category}`} className={classes.listSection}>
              <ul className={classes.ul}>
                <ListSubheader>{`${firstLetterUpper(category)}`}</ListSubheader>
                {cardsFiltered
